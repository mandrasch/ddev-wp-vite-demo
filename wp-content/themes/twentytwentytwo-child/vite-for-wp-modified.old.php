...
// Fork of https://github.com/kucrut/vite-for-wp/blob/main/vite-for-wp.php
// slightly modified for DDEV (we need to replace host 0.0.0.0 with site url),
// additions are marked with "Added by mandrasch"
...
function generate_development_asset_src( object $manifest, string $entry ): string {
  ...
  // Added by mandrasch, quick fix to use site_url instead of 0.0.0.0
  $manifest->data->origin = site_url().":5173";
  // eo added by mandrasch
...
function load_development_asset( object $manifest, string $entry, array $options ): ?array {
  ...
  // Added by mandrasch - quickfix, use site url instead of 0.0.0.0
  $src = str_replace("http://0.0.0.0",site_url().":5173", $src);
  // eo added by mandrasch
